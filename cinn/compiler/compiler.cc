...
#if defined(__IXCC_RTC__) || defined(__HIPCC_RTC__) || defined(__CUDACC_RTC__)
...
const char* IluvatarGetRuntimeSource(void* dev_ptr) {
  return kIxucaRuntimeSource;
}

C_Status IluvatarCompile(void* dev_ptr,
                         const char* code,
                         char* out_path,
                         size_t len) {
  uint64_t file_id = g_compile_counter.fetch_add(1);
  std::string file_prefix = "cinn_iluvatar_" + std::to_string(getpid()) + "_" +
                            std::to_string(file_id);

  std::string src_path = "/tmp/" + file_prefix + ".cu";
  std::string ll_path = "/tmp/" + file_prefix + ".ll";
  std::string obj_path = "/tmp/" + file_prefix + ".o";
  std::string cubin_path = "/tmp/" + file_prefix + ".cubin";

  bool keep_tmp = (std::getenv("FLAGS_keep_tmp") != nullptr);

  {
    std::ofstream src_file(src_path, std::ios::trunc);
    if (!src_file.is_open()) {
      std::cerr << "[Iluvatar] Failed to open temp file: " << src_path
                << std::endl;
      return C_Status::C_FAILED;
    }
    src_file << kIxucaRuntimeSource << "\n";
    src_file << code;
    src_file.close();
  }

  const char* ixuca_path_env = std::getenv("IXUCA_PATH");
  std::string ixuca_path =
      ixuca_path_env ? std::string(ixuca_path_env) : "/usr/local/corex";

  std::string clang_cmd = ixuca_path + "/bin/clang++";
  std::string llc_cmd = ixuca_path + "/bin/llc";
  std::string lld_cmd = ixuca_path + "/bin/lld";

  std::string cmd = clang_cmd +
                    " -std=c++20 -O2 -Wall"
                    " -I. -I" +
                    ixuca_path +
                    "/include"
  ...
  if (access(obj_path.c_str(), F_OK) != 0) {
    std::cerr << "[Iluvatar] Output file missing: " << obj_path << std::endl;
    cleanup(false);
    return C_Status::C_FAILED;
  }
